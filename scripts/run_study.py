...
def sanitize(value, private_root, runtime, workspace):
    if isinstance(value, dict):
        return {k: sanitize(v, private_root, runtime, workspace) for k, v in value.items()}
...
def collect(runtime, events_path):
    ...
    for path in sorted((runtime / "sessions").rglob("*.jsonl")):
        ...
        sessions.append({"id": meta.get("id"), "source": meta.get("source"), "rollout_sha256": digest(path), "model_effort": [{"turn_id": c.get("turn_id"), "model": c.get("model"), "effort": c.get("effort"), "sandbox": c.get("sandbox_policy")} for c in contexts], "usage": usage, "usage_event_count": len(counts), "usage_events": counts, "started_at": meta.get("timestamp"), "last_event_at": rows[-1].get("timestamp") if rows else None})
